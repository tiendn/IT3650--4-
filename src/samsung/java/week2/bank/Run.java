 ...
 */
public class Run {
	public final static int MAX_ACC = 50; // 
	/** 
	 *  List saving account: Acc
	 */
	public static SavingAccount[] Acc = new SavingAccount[MAX_ACC];
		...
		if ( n != MAX_ACC-1) { // Because The begin from 0 , so MAX_ACC = n + 1
			...
			if (!isExist(id)){
                ...
                // ******* Input Assets,date,rate ************/// 
		    		...
		    		do{
		    			input2 = new Scanner(System.in);
		    			System.out.println(" Enter day given ");
		    	        String date = input2.nextLine();
		    			dateGiven = convertSTD(date);
		    		}
		    		while (dateGiven.getTime() > dateGiven.getTime());
	...
	public static void displayInformation(){
		...
		if (index == -1){
			...
			System.out.println(" Day Interest Rate : " + Acc[index].getIRate());
		}
	}
	...
	public static void showMenu(){
		...
		case 3 : // partially withdraw
			...
			do {
				...
				Acc[index].withdrawPartially(money);
			}
			while ( money <= 0 || money > Acc[index].getAssets());
			showEnd(input1);
			showMenu();
			break;
